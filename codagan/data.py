...
def make_dataset(dir, fold, dataset_letter, sample):
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, root, sample, fold='train', dataset_letter='A', loader=default_loader, trim_bool=0, return_path=False, random_transform=False, channels=1, normalization='minmax'):

        imgs = make_dataset(root, fold, dataset_letter, sample)
        if len(imgs) == 0:
        ...
        if self.channels == 1:
            ...
            for i in range(img.shape[2]):
                ...
                if self.normalization == 'statistical':
                    tmp[i,:,:] = (tmp[i,:,:] - tmp[i,:,:].mean()) / (tmp[i,:,:].std() + 1e-10)
