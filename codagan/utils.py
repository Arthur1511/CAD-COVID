...
def get_all_data_loaders(conf, n_datasets, samples, augmentation, trim, test_train=False):
...
#     dataset_letters = ['A', 'C', 'D', 'E']
    dataset_letters = conf['dataset_letters']
    ...
    for i in range(n_datasets):
        ...
        train_loader = get_data_loader_folder(os.path.join(conf['data_root']), 'train', dataset_letters[i], batch_size, True, trim, num_workers, sample=samples[i], return_path=True, random_transform=augmentation[i], channels=conf['input_dim'])
...
def prepare_sub_folder(output_directory):
    ...
    if not os.path.exists(os.path.join(image_directory, 'probability')):
        print("Creating directory: {}".format(os.path.join(image_directory, 'probability')))
        os.makedirs(os.path.join(image_directory, 'probability'))

    if not os.path.exists(os.path.join(image_directory, 'isomorphic')):
        print("Creating directory: {}".format(os.path.join(image_directory, 'isomorphic')))
        os.makedirs(os.path.join(image_directory, 'isomorphic'))
...
def get_model_list(dirname, resume, key):

    if os.path.exists(dirname) is False:
        return None
    models = [os.path.join(dirname, f) for f in os.listdir(dirname) if
              os.path.isfile(os.path.join(dirname, f)) and key in f and ".pt" in f and str(resume) in f]
    if models is None:
        return None
    models.sort()
